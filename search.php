...
<?php include 'includes/header.php'; ?>
	...
	<title><?php echo $site['sitename'] ?> - Search</title>
				...
				<div class="container">
					<div class="row isotope-grid">
						 <?php
			       			...
			       			$stmt = $conn->prepare("SELECT COUNT(*) AS numrows FROM tb_barang WHERE namabarang LIKE :search");
			       			...
			       			if($row['numrows'] < 1){
			       				...
			       				<h1 style="font-family:Poppins-Regular;font-size:18px;">Pencarian <a href="#">'.$_POST['search'].'</a> Tidak Ditemukan</h1> </div>
			       				</div>';
			       			}
			       			else{
			       				echo '
			       				...
			       				<h1 style="font-family:Poppins-Regular;font-size:18px;">Hasil pencarian <a href="#">'.$_POST['search'].'</a> ditemukan '.$row['numrows'].' items</h1> </div>
			       				</div>
				                ...
				                try{
				                  ...
				                  $stmt = $conn->prepare("SELECT * FROM tb_barang WHERE namabarang LIKE :search LIMIT  ".$offset.", ".$tampil."");
				                  $stmt->execute(['search' => '%'.$_POST['search'].'%']);
				                  foreach($stmt as $row){
